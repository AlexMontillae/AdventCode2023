...
def adjacent_nums(list_of_str: list[str], i , j)-> int:
    ...
    if left.isalnum():
        total += right_or_left('left', list_of_str[i][j-3:j])
    if right.isalnum():
        total += right_or_left('right', list_of_str[i][j+1:j+4])
    if up.isalnum() and up_left.isalnum() and not up_right.isalnum():
        total += right_or_left('left', list_of_str[i-1][j-2:j+1])
    if up.isalnum() and up_right.isalnum() and not up_left.isalnum():
        total += right_or_left('right', list_of_str[i-1][j:j+3])
    if up.isalnum() and up_right.isalnum() and up_left.isalnum():
        total += right_or_left('middle', list_of_str[i-1][j-1:j+2])
    if up_left.isalnum() and not up.isalnum():
        total += right_or_left('left', list_of_str[i-1][j-3:j])
    if up_right.isalnum() and not up.isalnum():
        total += right_or_left('right', list_of_str[i-1][j+1:j+4])
    if up.isalnum() and not up_right.isalnum() and not up_left.isalnum():
        total += int(list_of_str[i-1][j])
...
def right_or_left(decision: str, iterable:str) -> int:
